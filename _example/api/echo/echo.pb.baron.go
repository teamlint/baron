...
// Version: v0.2.3-12-gfa1ce99-dirty
// Version Date: 2020-11-03T19:05:28+08:00
...
func DecodeHTTPEchoZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	...
	if len(buf) > 0 {
		...
		if err = unmarshaller.Unmarshal(buf, &req); err != nil {
			// if err = jsonpb.Unmarshal(buf, &req); err != nil {
			const size = 8196
			if len(buf) > size {
				buf = buf[:size]
			}
	...
	if OthersEchoStrArr, ok := queryParams["others"]; ok {
		OthersEchoStr := OthersEchoStrArr[0]

		var OthersEcho []string
		if len(OthersEchoStrArr) > 1 {
			OthersEcho = make([]string, len(OthersEchoStrArr))
			for i, v := range OthersEchoStrArr {
				converted := v
				if err != nil {
					return nil, errors.Wrapf(err, "couldn't decode OthersEcho from %v", OthersEchoStr)
				}
				OthersEcho[i] = converted
			}
		} else {
			err = json.Unmarshal([]byte(OthersEchoStr), &OthersEcho)
			if err != nil {
				OthersEchoStr = "[" + OthersEchoStr + "]"
			}
			err = json.Unmarshal([]byte(OthersEchoStr), &OthersEcho)
		}
		req.Others = OthersEcho
	}

	if JsonStrEchoStrArr, ok := queryParams["json_str"]; ok {
		JsonStrEchoStr := JsonStrEchoStrArr[0]

		var JsonStrEcho *wrappers.StringValue
		JsonStrEcho = &wrappers.StringValue{}

		// err = json.Unmarshal([]byte(JsonStrEchoStr), JsonStrEcho)
		// JsonStrEchoStr = "手动编码字符串"
		log.Printf("[decode] json_str = %v", JsonStrEchoStr)
		err = json.Unmarshal([]byte(JsonStrEchoStr), JsonStrEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "jsonpb couldn't decode JsonStrEcho from %v", JsonStrEchoStr)
		}
		req.JsonStr = JsonStrEcho
	}

	if JsonInt64EchoStrArr, ok := queryParams["json_int64"]; ok {
		JsonInt64EchoStr := JsonInt64EchoStrArr[0]

		var JsonInt64Echo *wrappers.Int64Value
		JsonInt64Echo = &wrappers.Int64Value{}
		err = json.Unmarshal([]byte(JsonInt64EchoStr), JsonInt64Echo)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode JsonInt64Echo from %v", JsonInt64EchoStr)
		}
		req.JsonInt64 = JsonInt64Echo
	}

	if MessageEchoStrArr, ok := queryParams["message"]; ok {
		MessageEchoStr := MessageEchoStrArr[0]

		var MessageEcho *Msg
		MessageEcho = &Msg{}
		err = json.Unmarshal([]byte(MessageEchoStr), MessageEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode MessageEcho from %v", MessageEchoStr)
		}
		req.Message = MessageEcho
	}

	if CreatedAtEchoStrArr, ok := queryParams["created_at"]; ok {
		CreatedAtEchoStr := CreatedAtEchoStrArr[0]

		var CreatedAtEcho *timestamp.Timestamp
		CreatedAtEcho = &timestamp.Timestamp{}
		err = json.Unmarshal([]byte(CreatedAtEchoStr), CreatedAtEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode CreatedAtEcho from %v", CreatedAtEchoStr)
		}
		req.CreatedAt = CreatedAtEcho
	}

	if AnyDataEchoStrArr, ok := queryParams["any_data"]; ok {
		AnyDataEchoStr := AnyDataEchoStrArr[0]

		var AnyDataEcho *any.Any
		AnyDataEcho = &any.Any{}
		err = json.Unmarshal([]byte(AnyDataEchoStr), AnyDataEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode AnyDataEcho from %v", AnyDataEchoStr)
		}
		req.AnyData = AnyDataEcho
	}

	if StructDataEchoStrArr, ok := queryParams["struct_data"]; ok {
		StructDataEchoStr := StructDataEchoStrArr[0]

		var StructDataEcho *_struct.Struct
		StructDataEcho = &_struct.Struct{}
		err = json.Unmarshal([]byte(StructDataEchoStr), StructDataEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode StructDataEcho from %v", StructDataEchoStr)
		}
		req.StructData = StructDataEcho
	}

	if EmptyDataEchoStrArr, ok := queryParams["empty_data"]; ok {
		EmptyDataEchoStr := EmptyDataEchoStrArr[0]

		var EmptyDataEcho *empty.Empty
		EmptyDataEcho = &empty.Empty{}
		err = json.Unmarshal([]byte(EmptyDataEchoStr), EmptyDataEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode EmptyDataEcho from %v", EmptyDataEchoStr)
		}
		req.EmptyData = EmptyDataEcho
	}

	if DurationDataEchoStrArr, ok := queryParams["duration_data"]; ok {
		DurationDataEchoStr := DurationDataEchoStrArr[0]

		var DurationDataEcho *duration.Duration
		DurationDataEcho = &duration.Duration{}
		err = json.Unmarshal([]byte(DurationDataEchoStr), DurationDataEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode DurationDataEcho from %v", DurationDataEchoStr)
		}
		req.DurationData = DurationDataEcho
	}

	if MaskDataEchoStrArr, ok := queryParams["mask_data"]; ok {
		MaskDataEchoStr := MaskDataEchoStrArr[0]

		var MaskDataEcho *field_mask.FieldMask
		MaskDataEcho = &field_mask.FieldMask{}
		err = json.Unmarshal([]byte(MaskDataEchoStr), MaskDataEcho)
		if err != nil {
			return nil, errors.Wrapf(err, "couldn't decode MaskDataEcho from %v", MaskDataEchoStr)
		}
		req.MaskData = MaskDataEcho
	}
...
func EncodeHTTPEchoZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
	...
	for _, v := range req.Others {
		values.Add("others", fmt.Sprint(v))
	}

	log.Printf("[json_str] = %v", req.JsonStr)
	tmp, err = json.Marshal(req.JsonStr)
	if err != nil {
		return errors.Wrap(err, "json failed to marshal req.JsonStr")
	}
	// tmp, err = jsonpb.Marshal(req.JsonStr)
	// if err != nil {
	// 	return errors.Wrap(err, "jsonpb.Marshal failed to marshal req.JsonStr")
	// }
	strval = string(tmp)
	values.Add("json_str", strval)

	log.Printf("[json_int64] = %v", req.JsonInt64)
	tmp, err = json.Marshal(req.JsonInt64)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.JsonInt64")
	}
	strval = string(tmp)
	values.Add("json_int64", strval)

	tmp, err = json.Marshal(req.Message)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.Message")
	}
	strval = string(tmp)
	values.Add("message", strval)

	tmp, err = json.Marshal(req.CreatedAt)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.CreatedAt")
	}
	strval = string(tmp)
	values.Add("created_at", strval)

	tmp, err = json.Marshal(req.AnyData)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.AnyData")
	}
	strval = string(tmp)
	values.Add("any_data", strval)

	tmp, err = json.Marshal(req.StructData)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.StructData")
	}
	strval = string(tmp)
	values.Add("struct_data", strval)

	tmp, err = json.Marshal(req.EmptyData)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.EmptyData")
	}
	strval = string(tmp)
	values.Add("empty_data", strval)

	tmp, err = json.Marshal(req.DurationData)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.DurationData")
	}
	strval = string(tmp)
	values.Add("duration_data", strval)

	tmp, err = json.Marshal(req.MaskData)
	if err != nil {
		return errors.Wrap(err, "failed to marshal req.MaskData")
	}
	strval = string(tmp)
	values.Add("mask_data", strval)

	r.URL.RawQuery = values.Encode()
	// Set the body parameters
	var buf bytes.Buffer
	// toRet := request.(*EchoRequest)
	// json
	...
	// 	},
	// }
	// encoder := jsonpb.NewEncoder(&buf)
	// if err := encoder.Encode(toRet); err != nil {
	// 	return errors.Wrapf(err, "[jsonpb] couldn't encode body as json %v", toRet)
	// }

	// protojson 没有序列化普通对象方法
	...
	if err != nil {
		return errors.Wrapf(err, "[protojson] couldn't encode body as json %v", req)
	}
